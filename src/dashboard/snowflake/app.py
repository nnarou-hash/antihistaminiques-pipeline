...
def load_gold(code):
    try:
        ...
        st.warning(f"Données non disponibles pour {code} — affichage de R06 par défaut")
        return read_sql(f"SELECT * FROM {MARTS}.GOLD_ML_R06")

@st.cache_data
def load_pred(code):
    try:
        return read_sql(f"SELECT * FROM {MARTS}.GOLD_PREDICTIONS_{code}")
    except Exception:
        return read_sql(f"SELECT * FROM {MARTS}.GOLD_PREDICTIONS_R06")
