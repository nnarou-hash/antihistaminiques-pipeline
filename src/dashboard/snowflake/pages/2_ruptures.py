...
@st.cache_data
def load_data():
    med = read_sql(f"SELECT * FROM {RAW}.ANSM.medicaments")
    rup = read_sql(f"SELECT * FROM {RAW}.ANSM.ruptures")

    # Ruptures 2025-2026
    try:
        rup_2026_raw = read_sql(f"SELECT * FROM {RAW}.ANSM.ruptures_ansm_2026")
    ...
    # Gold par classe ATC
    gold_dict = {}
    for atc in ['R06', 'R03', 'J01']:
        try:
            gold_dict[atc] = read_sql(f"SELECT * FROM {MARTS}.GOLD_ML_{atc}")
        except Exception:
            gold_dict[atc] = None

    # Fallback gold global
    try:
        gold_global = read_sql(f"SELECT * FROM {MARTS}.GOLD_ML_ADVANCED")
    except Exception:
        gold_global = read_sql(f"SELECT * FROM {MARTS}.GOLD_ML_R06")
