...
# =====================
# CHARGEMENT DONNEES — depuis Snowflake
# =====================
@st.cache_data
def load_openmedic():
    df = read_sql(f"SELECT * FROM {RAW}.OPENMEDIC.openmedic")
...
def load_gold(code):
    try:
        return read_sql(f"SELECT * FROM {MARTS}.GOLD_ML_{code}")
    except Exception:
        return read_sql(f"SELECT * FROM {MARTS}.GOLD_ML_R06")

@st.cache_data
def load_medicaments():
    df = read_sql(f"SELECT * FROM {RAW}.ANSM.medicaments")
...
st.sidebar.markdown("**Source :** CNAM OpenMedic (Snowflake)")

# =====================
# FILTRAGE
# =====================
df_f = df_om.copy()
df_f = df_f[df_f["atc3"].str.startswith(code_atc[:3], na=False)]
if annee_sel != 'Toutes':
...
with col_g2:
    ...
    df_evol_y = df_om[df_om["atc3"].str.startswith(code_atc[:3], na=False)].groupby("annee", as_index=False)["boites"].sum()
