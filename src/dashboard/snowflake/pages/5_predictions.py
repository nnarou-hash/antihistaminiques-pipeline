# src/dashboard/snowflake/pages/5_predictions.py
...
def load_data():
    pred = read_sql(f"SELECT * FROM {MARTS}.GOLD_PREDICTIONS_R06")
    gold = read_sql(f"SELECT * FROM {MARTS}.GOLD_ML_R06")
    return pred, gold
...
st.sidebar.markdown(f"**ROC-AUC CV :** 0.738")
...
st.subheader(f"📊 KPIs Prédictions — R06")
...
with col3:
    st.metric("Précision modèle", "56%",
              help="Accuracy du RF Classifier sur le jeu de test")
with col4:
    st.metric("ROC-AUC CV", "0.738",
...
with col_g1:
    ...
    if len(df_mois) > 0:
        ...
        fig_gauge = go.Figure(go.Indicator(
            ...
            title={'text': f"Risque rupture R06 — {mois_select}"},
            gauge={
                'axis': {'range': [0, 100]},
...
with col_g3:
    ...
    if 'pred_gram_next' in df_f.columns:
        fig_rv.add_trace(go.Scatter(
            x=df_f['annee_mois_str'],
...
st.subheader(f"🚨 Tableau des alertes — Mois à risque R06")
...
if len(df_alertes) > 0:
    cols_affich = ['annee_mois_str','proba_pct','gram_moy','nb_ruptures','target_rupture']
    if 'pred_gram_next' in df_alertes.columns:
        cols_affich.insert(2, 'pred_gram_next')
    st.dataframe(
        df_alertes[cols_affich].rename(columns={
            'annee_mois_str': 'Mois',
            'proba_pct': 'Probabilité (%)',
...
try:
    clf_path = 'models/R06/rf_classifier.joblib'
    ...
    fig_imp = px.bar(imp_top, x='importance', y='feature', orientation='h',
                     ...
                     title=f"Top {top_n} variables — RF Classifier R06")
    st.plotly_chart(fig_imp, use_container_width=True)

    st.caption("💡 ruptures_lag1 et les pollens sont les signaux les plus prédictifs pour les antihistaminiques")
...
try:
    clf = joblib.load('models/R06/rf_classifier.joblib')

    features_model = list(clf.feature_names_in_)
    features_disponibles = [f for f in features_model if f in gold.columns]

    df_roc = gold.dropna(subset=features_disponibles + ['target_rupture'])
