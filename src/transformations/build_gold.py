...
# ── Fusion des deux sources de ruptures ──────────────────────────────────────

def load_ruptures(classe_atc):
    """
    Charge et fusionne les ruptures CADA + ANSM 2026.
    Retourne un DataFrame agrégé par annee_mois_str avec :
      - nb_ruptures, nb_risques, target_rupture
    """

    # --- Source 1 : CADA ANSM (existant) ---
    ...
    rup_cada_filtre = rup_cada[
        rup_cada['code_atc'].str.startswith(classe_atc, na=False)
    ].copy()
    rup_cada_filtre['annee_mois_str'] = (
        rup_cada_filtre['date_evenement'].dt.to_period('M').astype(str)
    )
    # Normalisation : colonne statut → classification binaire
    rup_cada_filtre['is_rupture'] = (
        rup_cada_filtre['classification'] == 'rupture'
    ).astype(int)
    rup_cada_filtre['is_risque'] = (
        rup_cada_filtre['classification'] == 'risque'
    ).astype(int)

    # --- Source 2 : ANSM disponibilités 2026 (nouveau) ---
    ...
    if os.path.exists(ansm_path):
        ...
        rup_ansm_filtre = rup_ansm[
            rup_ansm['ATC4'].astype(str).str.startswith(classe_atc, na=False)
        ].copy()
        rup_ansm_filtre['annee_mois_str'] = (
            rup_ansm_filtre['date_debut'].dt.to_period('M').astype(str)
        )
        # Mapping statut ANSM → is_rupture / is_risque
        rup_ansm_filtre['is_rupture'] = rup_ansm_filtre['statut'].str.contains(
            'Rupture de stock', case=False, na=False
        ).astype(int)
        rup_ansm_filtre['is_risque'] = rup_ansm_filtre['statut'].str.contains(
            "Tension d'approvisionnement", case=False, na=False
        ).astype(int)
        print(f"  Source ANSM 2026 chargée : {len(rup_ansm_filtre)} lignes {classe_atc}")
    else:
    ...
    rup_agg['target_rupture'] = (
        (rup_agg['nb_ruptures'] + rup_agg['nb_risques']) > 0
    ).astype(int)

    print(f"  Ruptures fusionnées ({classe_atc}) : {len(rup_agg)} mois")
...
def build_gold(classe_atc='R06'):
    ...
    # ── Ruptures (CADA + ANSM 2026 fusionnées) ──
    rup_agg = load_ruptures(classe_atc)

    # ── Pollen par mois (inchangé) ──
    ...
    pollen_mois = pollen.groupby('annee_mois_str').agg(
        ...
        aulne_moy=('aulne','mean'),
        armoise_moy=('armoise','mean'),
        olivier_moy=('olivier','mean'),
    ...
    # ── Open Medic filtré par classe ATC (inchangé) ──
